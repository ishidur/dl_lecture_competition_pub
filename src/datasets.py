...
class Sequence(Dataset):
    def __init__(
        ...
        self.load_gt = True  # TODO: remove at final submission
        self.transforms = transforms
        if self.mode == "test":
            # assert load_gt == False  # TODO: recover at final submission
    ...
    def __getitem__(self, idx):
        ...
        for key_t, value_t in self.transforms.items():
            if key_t == "hflip":
...
class DatasetProvider:
    def __init__(
        ...
        exc_seq = config.get("exclude_seqs", [])
        seqs = [seq for seq in available_seqs if seq not in exc_seq]

        transforms = dict()
        if config.get("horizontal_flip", None):
            p_hflip = config.horizontal_flip
            assert p_hflip >= 0 and p_hflip <= 1
            transforms["hflip"] = p_hflip
